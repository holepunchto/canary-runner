class Result {
  ...
  addFailureBeforeTest (runOverview) {
    const repoName = `${runOverview.repo}#${runOverview.branch}`
    const gitHash = runOverview.gitHash
    const locklessInfo = runOverview.lockessRun ? ' (without lockfile)' : ''
    this.reposWithFailuresBeforeTests.set(
      repoName, `${repoName}${locklessInfo} had an unexpected failure during preliminary step '${runOverview.step}'\nCommit hash: ${gitHash}\n\n${runOverview.stderr}\n`
    )
  }
  ...
  add (runOverview) {
    ...
    const [preamble, ...allTestsArrayed] = runOverview.stdout.split(
      /TAP version [0-9]+/
    )
    const allTestsRaw = allTestsArrayed.join('\n') // merge allTestsArrayed into single string because it can contain nested tests with a `TAP version`

    const lockInfo = runOverview.locklessRun ? ' (without lockfile)' : ''
    const repoInfo = `${preamble.trim().split('\n')[0]}${lockInfo} for branch ${runOverview.branch} (commit hash ${runOverview.gitHash})`
    if (!allTestsRaw) { // Error before any tests got to run
      this.addFailureBeforeTest(runOverview)
      return
    }
  ...
  getSummary ({ detailed = false } = {}) {
    ...
    if (this.reposWithFailuresBeforeTests.size > 0) {
